...
def test_findUser(requests_mock, mockOAuthSession, getTestConfig):
    ...
    assert user[2] == '1671151d-ac48-4b4d-a204-c858c3bf5d86'
    assert user[3] == None
    assert user[4] == 'success'
    
    #identifier, sourceSystemName, principalId, sourceSystemId, status
...
def test_findUserCorrelationData(requests_mock, mockOAuthSession, getTestConfig):
    ...
    assert type(user) is pandas.core.series.Series
    assert user[2] == '1671151d-ac48-4b4d-a204-c858c3bf5d86'
    assert user[3] == '069cc81f-395d-4272-86f2-99b54d8fbc29'   
    assert user[4] == 'success'
